    ...
    <div class="container-fluid">
      <div class="row mb-2">
        ...
        <div class="col-3 align-self-center">
            <a href="{{ route("barang-masuk.create") }}" class="btn btn-secondary float-right"><i class="fa fa-plus mr-2"></i> Tambah List </a>
        </div>
      </div><!-- /.row -->
    </div><!-- /.container-fluid -->
  </div>
                            ...
                            <tbody>
                                @foreach ($data_barang as $data_barang )
                                    ...
                                    <tr>
                                        ...
                                        <td class="text justify-content-center">{{ $data_barang->vendor }}</td>
                                        ...
                                        <td class="text-center">
                                            <div class="btn btn-toolbar justify-content-center">
                                                <form action="{{ route('barang-masuk.destroy', $data_barang->id) }}" method="post" onsubmit=" return confirm('Are you sure want to delete this item?')">
                                                    @csrf
                                                    @method('DELETE')
                                                    <button class="btn btn-sm mr-2 btn-danger"><i class=" fa fa-trash" ></i> Delete</button>
                                                </form>
                                                <a href="{{ route('barang-masuk.edit', $data_barang->id) }}" method=post class="btn btn-sm mr-2 btn-warning text-white"><i class="fa fa-edit mr-2"></i>Edit</a>
                                            </div>
                                        </td>
                                    </tr>
                                @endforeach
