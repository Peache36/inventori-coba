    ...
    <div class="row">
        <div class="col-md-12">
            <div class="card">
                <!-- /.card-header -->
                <div class="card-body">
                    <form action="{{ route("barang-keluar.update", $data_barang->id) }}" method="post">
                        @csrf
                        @method('PUT')
                        <div class="col-6 d-inline-block">
                            <div class="form-group mb-3">
                                ...
                                <select class="form-control" name="barang_id" id="barang_id">
                                    ...
                                    @foreach ($data_keluar as $data_keluar )
                                            <option value={{ $data_keluar->id }} >{{ $data_keluar->nama_barang }}</option>
                                    @endforeach
                                  </select>
                                @error('barang_id')
                        ...
                        <div class="form-group mb-3">
                            ...
                            <input type="text" name="no_do" class="form-control rounded-0 @error('no_do') is-invalid @enderror" id="no_do" placeholder="Nomor Do ..." value="{{ $data_barang->no_do }}" required>
                            @error('no_do')
                                <small class="text-danger">{{ $message }}</small>
                            @enderror
                        </div>
